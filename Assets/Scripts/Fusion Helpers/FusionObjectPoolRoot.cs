...
namespace FusionExamples.FusionHelpers
{
	...
	public class FusionObjectPoolRoot : MonoBehaviour, INetworkObjectPool
	{
		...
		public NetworkObject AcquireInstance(NetworkRunner runner, NetworkPrefabInfo info)
		{
			...
			CLog.LogError("No prefab for " + info.Prefab);
			return null;
		}
		...
		public void ReleaseInstance(NetworkRunner runner, NetworkObject no, bool isSceneObject)
		{
			...
			CLog.Log($"Releasing {no} instance, isSceneObject={isSceneObject}");
			if (no.gameObject.GetComponent<MinaVIPPowerup>())//DABROS316
			...
			{
							runner.Despawn(no);
			//	no.gameObject.GetComponent<MisilPowerup>().explosion.gameObject.SetActive(true);
				//no.gameObject.GetComponent<MisilPowerup>().explosion.Play();
			}
			*/
