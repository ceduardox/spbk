...
public class AudioManager : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Invoke("turnOffAudio",.5f);
	}
	
	void turnOffAudio()
    {
		//CLog.Log("ESTO ES LA COSA: " + GameLauncher.instance.modeServerDedicado);
		...
		else
        {
			if (GameLauncher.instance.modeServerDedicado)
			{
				...
				if(SpinStore.instance) SpinStore.instance.turnOffCameras();
			}
	...
	public static void Play(string clip, AudioMixerGroup mixerTarget, Vector3? position = null)
	{
		...
		{
			CLog.LogWarning($"AudioClip '{clip}' not present in audio bank");
		}
	}
	...
	public static bool PlayMusic(string music)
	{
		...
		{
			Debug.LogWarning("PlayMusic omitido: AudioManager o musicSource no disponible.");
			return true;
		}
		...
		if (string.IsNullOrEmpty(music) == false)
		{
			...
			{
				//if(audio!= Instance.musicSource.clip)
			...
			{
				CLog.LogWarning($"AudioClip '{music}' not present in music bank");
				CLog.LogWarning($"ASIGNANDO AUDIO ALEATORIO");
			}
		}
		return true;
	}
	...
	{
		if (Instance != null && Instance.musicSource)
		{
			Instance.musicSource.Stop();
			Instance.musicSource.clip = null;
		}
		else
			Debug.LogWarning("StopMusic omitido: Instance.musicSource is Null");
	}
	...
	public static void SetVolumeMaster(float value)
	{
		Instance.masterMixer.SetFloat(mainVolumeParam, ToDecibels(value));
			Debug.Log("AudioManager está configurando el volumen");
